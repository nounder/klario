...
import type { AppState, Bounds, EraserToolState, Node, StrokePoint } from "../types"

export const NodeType = null // Eraser doesn't create nodes

export const initialState: EraserToolState = {
  width: 20,
  currentPath: [],
    ...
    if (i > 0) {
      ...
      if (
        segmentIntersectsRect(
          prevPoint,
          point,
          { x: left, y: top, width: node.bounds.width, height: node.bounds.height }
        )
      ) {
        return true
      }
...
function segmentIntersectsRect(
  ...
  return (
    ...
    lineSegmentsIntersect(p1, p2, { x: right, y: top }, { x: right, y: bottom }) ||
    lineSegmentsIntersect(p1, p2, { x: left, y: top }, { x: right, y: top }) ||
    lineSegmentsIntersect(p1, p2, { x: left, y: bottom }, { x: right, y: bottom })
  )
}
...
export function renderSettings(_props: {
  ...
  const [state, setState] = createStore<EraserToolState>(initialState)
